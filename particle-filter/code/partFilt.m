...
% clear
% close all
proDir = '/Users/ansonliang55/Dropbox/UOT/CSC2503/matlab/pigTrackHandout/';
cd([proDir]);
seqRoot = '/Users/ansonliang55/Dropbox/UOT/CSC2503/matlab/pigeon/';
if ~exist('seqRoot')
  % CHANGE THIS
  seqRoot = '/Users/ansonliang55/Dropbox/UOT/CSC2503/matlab/pigeon/';
end
...
for pigMode = 1; % 1 denotes grey pigeon, 2 denotes red pigeon, 3 clone
                   
  marking = 0;  % 0 for back, 1 for head
  ...
  for frameNum = frameNum0:seqParams.frameSkip:seqParams.frameEnd
    ...
    a = sprintf('results/%s%dmark%dfrm%s.mat', outPrefix, pigMode, ...
        marking, num2strPad(frameNum, seqParams.padNum), 'sampDist')
    c = sprintf('results/%s%dmark%dfrm%s.mat',outPrefix, pigMode, ...
