...
if isunix
  ...
  codeDir = '/Users/ansonliang55/Dropbox/UOT/CSC2503/matlab/A1handout/';
  ...
  codeDir = 'path to your assignment working directory called A1handout';
  dataDir = [codeDir, 'ppmImages\'];
  chromeDir = [dataDir, 'chrome\'];
end
...
for useImageSet = 3 % 1:6  or just choose one image set, e.g. 3
  ...
  imDataRED=zeros(numPixels, nDirChrome);
  imDataGREEN=zeros(numPixels, nDirChrome);
  imDataBLUE=zeros(numPixels, nDirChrome);
  
  % re-read the ppm file to get data from every channel
  for i=1:nDirChrome
    fname = [imageDir,name,'.',num2str(i-1),'.ppm'];
    RGBim = ppmRead(fname);
    imDataRED(:,i) = reshape(RGBim(:,:,1), [numPixels, 1]);
    imDataGREEN(:,i) = reshape(RGBim(:,:,2), [numPixels, 1]);
    imDataBLUE(:,i) = reshape(RGBim(:,:,3), [numPixels, 1]);
  end
  
  %crop the image with the mask
  albedoRGBCrop = albedo(mask,1:3);
  imDataRedCrop = imDataRED(mask,1:nDirChrome);
  imDataGreenCrop = imDataGREEN(mask,1:nDirChrome);
  imDataBlueCrop = imDataBLUE(mask,1:nDirChrome);
  nCropPixels=size(imDataRedCrop,1);
 
  %reuse the function from question 2
  [~, albedoRGBCrop(:,1)] = fitReflectance(imDataRedCrop, Lchrome);
  [~, albedoRGBCrop(:,2)] = fitReflectance(imDataGreenCrop, Lchrome);
  [~, albedoRGBCrop(:,3)] = fitReflectance(imDataBlueCrop, Lchrome);
  
  % Unpack albedos estimated from within the mask
  % into imsize sized images, and display.
  albedo(mask,:) = albedoRGBCrop;
  ...
  %crop the image 9-12 using the mask
  imDataCrop = imData(mask,(nDirChrome+1):nDir);
  
  %calculate Lrec using least square method
